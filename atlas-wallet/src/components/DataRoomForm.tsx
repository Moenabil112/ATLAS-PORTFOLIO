...
export default function DataRoomForm() {
  ...
  const validate = (): boolean => {
    ...
    required.forEach((field) => {
      if (!form[field]) { newErrors[field] = true; valid = false }
    })
    if (!form.confirmed) { newErrors.confirmed = true; valid = false }
    setErrors(newErrors)
    return valid
  }
    ...
    return (
      <div className="animate-fade-in space-y-6">
        ...
        <div className="border border-graphite-mid bg-graphite/30 p-6">
          <h2 className="text-xl font-medium text-ivory mb-3">{t.ndaTitle}</h2>
  ...
  return (
    <div>
      <h2 className="text-xl font-medium text-ivory mb-2">{t.accessFormTitle}</h2>
      ...
      <form onSubmit={handleSubmit} className="max-w-2xl space-y-5" noValidate>
        ...
        <div className="flex items-start gap-3">
          ...
          />
          <label htmlFor="confirm-dr" className="text-xs text-sand/60 leading-relaxed cursor-pointer">
            {t.form.confirm}
          </label>
        </div>
